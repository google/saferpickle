...
def optimize(origpickle, protocol=2):
  ...
  ast = Import(pickle.loads if PY2 else pickle._loads)(
      Import(zlib.decompress)(data)
  )
...
class SetAttributes(PickleBase):
  ...
  It will call __dict__.update with the given keyword arguments and return the
  object.
  """
    ...
    def _serialize(self, pickler):
      if self.cache:
        ...
        if tup in pickler.memo:
          ...
          pickler.write(
              pickle.GLOBAL + self.module + NEWLINE + self.name + NEWLINE
          )
...
class Sequence(PickleBase):
  ...
  if `reversed` is True then the first object will be returned instead of the
  last object.
  """
  ...
  def __init__(self, *objects, **kwargs):
    ...
    if kwargs:
      raise TypeError(
          "__init__ got too many keyword arguments: {}".format(kwargs)
      )
    ...
    for i, obj in enumerate(objects):
      if isinstance(obj, Sequence) and (
          obj.reversed
          == self.reversed  # both sequences have the same direction
          or (
              self.reversed and i
          )  # or this is not the first item in a reversed sequence
          or (not self.reversed and i != len(objects) - 1)
      ):  # or this is not the last item in a non-reversed sequence
...
class Assign(PickleBase):
  """This class stores `value` in `varname`.

  This is implemented as pushing the value on to the memo. This returns `value`.
  """
...
class Load(PickleBase):
  ...
  This is implemented by getting the value from the memo. This returns `value`
  """
  ...
  def _serialize(self, pickler):
    if self.varname not in pickler.memo:
      raise ValueError(
          "attempted to use variable {0} but it hasn't been defined yet".format(
              self.varname
          )
      )
...
def Ternary(conditional, true_value, false_value):
  """A simple ternary statement.

  Due to the limitations of pickling both branches will be executed But it is
  possible to have a conditional final result.
  """
...
def DeclareModule(name, retval=True):
  ...
  This creates an empty module and inserts it in the sys.modules namespace, if
  retval is True then the module will be returned else sys.modules will be
  returned
  """
  ...
  val = SetItem(Imports("sys", "modules"), name, module_fn(name))
...
def DefineModule(name, code, executor=Exec):
  ...
  """
  return executor(
      code, globals=Imports(name, "__dict__"), filename="<{0}>".format(name)
  )
...
def GetModule(name):
  ...
  Note that, if you ever need something contained in a module, it is more
...
def Module(name, code, retval=True, executor=Exec):
  ...
  references between modules are problematic, the declaring and defining has to
  be ordered manually.
...
def ExecTranspile(
    string, foreign=(), globals=Globals(), locals=None, filename="<pickle>"
):
...
class TransPickler(ast.NodeVisitor):
  ...
  def visit_Dict(self, node):
    ...
    return dict(zip(self.visit_list(node.keys), self.visit_list(node.values)))
  ...
  def visit_Exec(self, node):
    return Exec(
        self.visit(node.body), self.visit(node.globals), self.visit(node.locals)
    )
  ...
  def visit_BinOp(self, node):
    return Import(self.BINOP_MAP[node.op.__class__])(
...
def ExecAst(string, globals=Globals(), locals=None, filename="<pickle>"):
  ...
  The mechanism used for this is compiling the code to an AST, serializing this
  AST and then
...
class PyAstCompiler(ast.NodeTransformer):
  ...
  compresses in a more optimized format due to it calling the ast constructors
  directly.
  ...
  def generic_visit(self, node):
    ...
    for field, old_value in ast.iter_fields(node):
      if isinstance(old_value, list):
        old_value[:] = [
            self.visit(i) if isinstance(i, ast.AST) else i for i in old_value
        ]
...
def optimize_puts(p):
  ...
  replmap = dict(
      (key, i)
      for i, key in enumerate(sorted(counter.keys(), key=lambda x: -counter[x]))
  )

...
class FakeClassType(type):
  """The metaclass used to create fake classes.

  To support comparisons between fake classes and :class:`FakeModule` instances
  custom behaviour is defined here which follows this logic:
  ...
  Using this behaviour, ``==``, ``!=``, ``hash()``, ``isinstance()`` and
  ``issubclass()``
  are implemented allowing comparison between :class:`FakeClassType` instances
  and :class:`FakeModule` instances to succeed if they are pretending to be in
  the same
  place in the python module hierarchy.

  To create a fake class using this metaclass, you can either use this metaclass
  directly or
  inherit from the fake class base instances given below. When doing this, the
  module that
  this fake class is pretending to be in should be specified using the *module*
  argument
  when the metaclass is called directly or a :attr:``__module__`` class
  attribute in a class statement.
...
class FakeClassFactory(object):
  """Factory of fake classses.

  It will create fake class definitions on demand based on the passed arguments.
  """
  ...
  def __init__(self, special_cases=(), default_class=FakeStrict):
    ...
    as special cases during the fake unpickling process. This way you can
    specify custom methods
    and attributes on these classes as they're used during unpickling.

    *default_class* should be a FakeClassType instance which will be subclassed
    to create the
    necessary non-special case fake classes during unpickling. This should
    usually be set to
    :class:`FakeStrict`, :class:`FakeWarning` or :class:`FakeIgnore`. These
    classes have
    :meth:`__new__` and :meth:`__setstate__` methods which extract data from the
    pickle stream
    and provide means of inspecting the stream when it is not clear how the data
    should be interpreted.

    As an example, we can define the fake class generated for definition bar in
    module foo,
    ...
    Alternatively they can also be instantiated using :class:`FakeClassType`
    directly::
       special_cases = [FakeClassType(c.__name__, c.__bases__, c.__dict__,
       c.__module__)]
    """
  ...
  def __call__(self, name, module):
    ...
    This class will either be one of the special cases in case the name and
    module match,
    or a subclass of *default_class* will be created with the correct name and
    module.
...
class FakeModule(types.ModuleType):
  ...
  *name* is the name of the module and should be a ``"."`` separated
  ...
  Else if the other object does not have ``other.__module__`` set, they are
  equal if:
  ...
  ``issubclass()``
  are implemented allowing comparison between :class:`FakeClassType` instances
  and :class:`FakeModule` instances to succeed if they are pretending to bein
  the same
  place in the python module hierarchy.
...
class FakePackageLoader(object):
  """A :term:`loader` of :class:`FakePackage` modules.

  When added to :data:`sys.meta_path` it will ensure that any attempt to import
...
class FakeUnpickler(pickle.Unpickler if PY2 else pickle._Unpickler):
  """A forgiving unpickler.

  On uncountering references to class definitions in the pickle stream which it
  cannot locate, it will create fake classes and if necessary fake modules to
  house them in. Since it still allows access to all modules and builtins, it
  should only be used to unpickle trusted data.
  ...
  *class_factory* can be used to control how the missing class definitions are
  created. If set to ``None``, ``FakeClassFactory((), FakeStrict)`` will be
  used.

  In Python 3, the optional keyword arguments *encoding* and *errors* can be
  used
  to indicate how the unpickler should deal with pickle streams generated in
  python
  2, specifically how to deal with 8-bit string instances. If set to "bytes" it
  will
  load them as bytes objects, otherwise it will attempt to decode them into
  unicode
  ...
  if PY2:
    ...
    def __init__(
        self, file, class_factory=None, encoding="bytes", errors="strict"
    ):
    ...
    def __init__(
      ...
      super().__init__(
          file, fix_imports=False, encoding=encoding, errors=errors
      )
      self.class_factory = class_factory or FakeClassFactory()
...
class SafeUnpickler(FakeUnpickler):
  ...
  It will create fake classes for any references to class definitions in the
  pickle stream. Further it can block access to the extension registry making
  this unpickler safe to use on untrusted data.

  *file* is the :term:`binary file` to unserialize.

  The optional keyword arguments are *class_factory*, *safe_modules*,
  *use_copyreg*,
  *encoding* and *errors*. *class_factory* can be used to control how the
  missing class
  definitions are created. If set to ``None``, ``FakeClassFactory((),
  FakeStrict)`` will be
  used. *safe_modules* can be set to a set of strings of module names, which
  will be
  regarded as safe by the unpickling process, meaning that it will import
  objects
  from that module instead of generating fake classes (this does not apply to
  objects
  in submodules). *use_copyreg* is a boolean value indicating if it's allowed to
  use extensions from the pickle extension registry (documented in the
  :mod:`copyreg`
  module).
  ...
  def __init__(
    ...
    self.unsafe_modules = set(unsafe_modules)
  ...
  def find_class(self, module, name):
    ...
    # Support submodule prefix matching for safe modules AND exact fully qualified class matches
    is_safe_mod = module in self.safe_modules or any(
        module.startswith(safe + ".") for safe in self.safe_modules
    )
    is_safe_class = f"{module}.{name}" in self.safe_modules

    if not is_safe_mod and not is_safe_class and not sys.modules.get(module):
      try:
        __import__(module)
      except Exception:
        logger.warning("Unknown module/class imported: %s", module)

    if is_safe_mod or is_safe_class:
      # Resolve base safe module name to check sys.modules
      base_safe_mod = module
      if not is_safe_class and module not in self.safe_modules:
        for safe in self.safe_modules:
          if module.startswith(safe + "."):
            base_safe_mod = safe
            break

      if not sys.modules.get(base_safe_mod):
        return self.class_factory(name, module)

      # Try to import the actual submodule dynamically
      try:
  ...
  def load_build(self, *unused_args):
    ...
    try:
      ...
      # and state is a dict, update __dict__.
      if isinstance(state, dict):
        logger.info("Updating __dict__ of %s with state", type(inst))
        inst.__dict__.update(state)
      else:
        logger.warning(
            "Cannot update __dict__ of %s with non-dict state of type %s",
            type(inst),
            type(state),
        )
  ...
  def load_newobj(self, *unused_args):
    ...
    try:
      ...
      logger.warning("Failed to newobj %s with %s: %s", cls, args, e)
  ...
  def load_newobj_ex(self, *unused_args):
    ...
    try:
      ...
      logger.warning(
          "Failed to newobj_ex %s with %s, %s: %s", cls, args, kwargs, e
      )
      obj = self.class_factory("failed_newobj_ex", "picklemagic")()
    self.stack.append(obj)
  ...
  def load_stack_global(self, *args):
    name = self.stack.pop()
...
class SafePickler(pickle.Pickler if PY2 else pickle._Pickler):
  ...
  Due to reasons unknown, pythons pickle implementation will normally check if a
  given class actually matches with the object specified at the __module__ and
  __name__ of the class. Since this check is performed with object identity
  instead of object equality we cannot fake this from the classes themselves,
  and we need to override the method used for normally saving classes.
  """
...
def load(file, class_factory=None, encoding="bytes", errors="errors"):
  ...
  *class_factory* can be used to control how the missing class definitions are
  ...
  return FakeUnpickler(
      file, class_factory, encoding=encoding, errors=errors
  ).load()
...
def safe_load(
  ...
  and return the reconstitutded object hierarchy specified therein, substituting
  any
...
def safe_dump(obj, file, protocol=pickle.HIGHEST_PROTOCOL):
  """A convenience function wrapping SafePickler.

  It functions similarly to pickle.dump
  """
...
def safe_dumps(obj, protocol=pickle.HIGHEST_PROTOCOL):
  ...
  It functions similarly to pickle.dumps
  """
  file = StringIO()
  SafePickler(file, protocol).dump(obj)
...
def fake_package(name):
  """Mounts a fake package tree with the name *name*.

  This causes any attempt to import module *name*, attributes of the module or
  submodules will return a :class:`FakePackage` instance which implements the
  same behaviour. These :class:`FakePackage` instances compare properly with
  :class:`FakeClassType` instances allowing you to code using FakePackages as if
  the modules and their attributes actually existed.

  This is implemented by creating a :class:`FakePackageLoader` instance with
  root *name*
  and inserting it in the first spot in :data:`sys.meta_path`. This ensures that
  importing the
  module and submodules will work properly. Further the :class:`FakePackage`
  instances take
  care of generating submodules as attributes on request.

  If a fake package tree with the same *name* is already registered, no new fake
  package
...
def remove_fake_package(name):
  ...
  loaders = [
      ...
      if isinstance(i, FakePackageLoader) and i.root == name
  ]

...
from typing import IO, Any, BinaryIO, Callable, Dict, Iterator, Optional, Set, Tuple

from absl import logging

from lib import config, constants, exceptions, utils
                    ...
                    match charcode:
                        case c if c in constants.OPCODES_4BYTE_LEN:
                            length = int.from_bytes(
                                pickle_file.read(4), byteorder="little"
                            )
                        case c if c in constants.OPCODES_8BYTE_LEN:
                            length = int.from_bytes(
                                pickle_file.read(8), byteorder="little"
                            )
                        case c if c in constants.OPCODES_1BYTE_LEN:
                            length = int.from_bytes(
                                pickle_file.read(1), byteorder="little"
                            )
                        case _:
                            length = 0
...
def generate_ops_from_file(
    ...
    num_workers = utils.get_optimal_workers(
        pickle_length
    )  # pyrefly: ignore[bad-argument-type]

    if (
        pickle_length
        < constants.MIN_SIZE_FOR_CHUNKING  # pyrefly: ignore[unsupported-operation]
        or not utils.is_sys_executable_patched()
    ):
        if shm_name:
            shm = shared_memory.SharedMemory(name=shm_name)
            pickle_bytes = bytes(
                shm.buf[:pickle_length]
        ...
        for chunk_index in range(num_workers):
            ...
            if (
                chunk_start_size < pickle_length
            ):  # pyrefly: ignore[unsupported-operation]
...
def generate_ops(
    ...
    try:
        try:
            for _, operand in _custom_genops(
                pickle_bytes
            ):  # pyrefly: ignore[bad-argument-type]
                if operand is None:
                    continue
...
def categorize_strings(
    ...
    if use_picklemagic and isinstance(filtered_output, io.StringIO):
        ...
        safe, unsafe, suspicious, unknown = _categorize_genops(
            filtered_output
        )  # pyrefly: ignore[bad-argument-type]
...
def _categorize_genops(
    ...
    for line in filtered_output:
        ...
        safe_match = any(
            safe_string in line_in_lowercase for safe_string in constants.SAFE_STRINGS
...
def genops_scan(
    ...
    resolved_pickle_length = (
        pickle_length
        if pickle_length is not None
        else len(pickle_bytes)  # pyrefly: ignore[bad-argument-type]
    )
    if shm_name:
...
def _is_unsafe_archive_member(name: str) -> bool:
    """Detects path-traversal and absolute-path archive member names.

    This catches classic zip-slip and tar-slip payloads, including Windows-style
    separators and drive letters that a naive ``..`` / leading-slash check misses.

    Args:
      name: The archive member name to inspect.

    Returns:
      True if the member name could escape the extraction directory.
    """
    if not name:
        return False
    if name.startswith(("/", "\\")):
        return True
    # Windows drive-letter absolute paths (e.g. "C:\\...") and UNC-style paths.
    if re.match(r"^[a-zA-Z]:", name):
        return True
    # Normalize separators and check for a traversal component.
...
def _security_scan_internal(
    ...
    try:
        is_pickle = utils.is_pickle_file(stream, check_magic_bytes=check_magic_bytes)
    ...
    if pickle_length >= constants.MIN_SIZE_FOR_CHUNKING:
        try:
            ...
            if isinstance(stream, io.BytesIO):
                shm.buf[
                    :pickle_length
                ] = stream.getbuffer()[  # pyrefly: ignore[unsupported-operation]
                ...
                try:
                    while True:
                        ...
                        if not chunk:
                            break
                        shm.buf[offset : offset + len(chunk)] = (
                            chunk  # pyrefly: ignore[unsupported-operation]
                        )
                        offset += len(chunk)
                finally:
                    stream.seek(current_pos)
    ...
    **kwargs: Any,
    ...
    if strict_check and allow_unsafe:
        ...
        scan_scores = security_scan(
            scan_source,
            force_scan=force_scan,
            check_magic_bytes=check_magic_bytes,  # pyrefly: ignore[bad-argument-type]
        )
            ...
            for method_name, custom_func in methods_to_patch.items():
                if hasattr(module, method_name):
                    if method_name not in _ORIG_METHODS_BEFORE_HOOKING[hookable_mod]:
                        _ORIG_METHODS_BEFORE_HOOKING[hookable_mod][method_name] = (
...
def loads(
    ...
    **kwargs: Any,
    ...
    """Custom loads function to security scan before loading pickle data.

    This function can be used as a replacement for pickle.loads, providing
    security scan features on serialized pickle bytes.

    Args:
      pickle_data: The serialized pickle data to load.
      allow_unsafe: If True, allow unsafe pickle files to be loaded.
      strict_check: If True, perform a strict security check on the pickle file.
      report_only: If True, only report errors and do not raise them.
      force_scan: If True, force scan even if the file is not a pickle file.
      ...
      check_magic_bytes: Whether to perform magic byte checks to fast path reject
        files.
      *args: Additional positional arguments to pass to pickle.loads.
      **kwargs: Additional keyword arguments to pass to pickle.loads.

    Returns:
      The unpickled object or None if the pickle data is unsafe and report_only
      is True.

    Raises:
      UnsafePickleDetectedError: If the pickle data is unsafe.
    """
    return _scan_and_load(
        pickle_data,
        allow_unsafe,

...
import types
import zipfile
from typing import IO, BinaryIO, Callable, Dict, FrozenSet, Generator, Set, Tuple, cast
...
ARGS_REGEX = re.compile(r"\<class '(.*?)'\>.*?unexpected arguments [({](.*)[)}]")
...
def get_optimal_workers(file_size: int) -> int:
    ...
    scaled_workers = largest_workers + int(math.log(file_size / largest_threshold, 2))
...
def resolve_library_modules_from_results(
    ...
    for module_name, members in importable_modules.items():
        ...
        if not found_member_in_candidates and module_name not in resolved_candidates:
...
def categorize_picklemagic(
    ...
    for line in lines:
        ...
        if "Unsafe module/class invoked:" in line:
            match = re.search(r"Unsafe module/class invoked:\s*([a-zA-Z0-9_.]+)", line)
            if match:
